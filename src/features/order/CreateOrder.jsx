...
function CreateOrder() {
  ...
  const totalPrice = totalCartPrice + totalCartPrice * priorityPrice;
      ...
      <Form method="POST">
        ...
        <div className="relative mb-5 flex flex-col gap-2 sm:flex-row sm:items-center">
          ...
          <div className="grow">
            ...
            {addressStatus === "error" && (
              <p className="mt-2 rounded-md bg-red-100 p-2 text-xs text-red-700">
                {errorAddress + ". Please make sure to fill this field."}
              </p>
            )}
          </div>

          {!position.latitude && !position.longitud && (
            <span className="absolute -top-0.5 right-[3px] z-50 sm:top-[2.5px] md:top-[5px]">
              <Button
                disabled={isLoadingAddress}
                type="small"
                onClick={(e) => {
